...
# Firebaseサーバープログラムの仕組み解説（司令塔の仕事内容）

このプログラムは、Googleのサーバー上で24時間動き続ける、アプリの「頭脳」です。アプリから送られてくる様々な「お願い」を処理しています。ここでは、5つの主要な仕事内容を解説します。

### 1. `registerUser`（会員登録）

- **仕事内容:** 新しいユーザーがアプリを使い始めたときに、その人の「会員証」を作成します。
- **アプリとの連携:**
    1. ユーザーがアプリで自分の名前を入力し、「登録」ボタンを押します。
    2. アプリが「この名前で会員証を作ってください」と、この`registerUser`という命令を呼び出します。
    3. サーバーは、データベースの`users`という名簿に、その人の名前とユニークなIDを記録します。

### 2. `joinOrCreateRoom`（部屋の割りふり）

- **仕事内容:** ユーザーを「合言葉」で指定された部屋に入室させます。もし誰も使っていない部屋なら、新しい部屋として用意します。
- **アプリとの連携:**
    1. ユーザーがアプリで「合言葉」を入力し、「入室」ボタンを押します。
    2. アプリが「この合言葉の部屋に入れてください」と、この`joinOrCreateRoom`という命令を呼び出します。
    3. サーバーは、データベースの`rooms`という部屋リストをチェックします。
        - **もし部屋があれば:** その部屋にユーザーを追加します。
        - **もし部屋がなければ:** 新しくその合言葉の部屋を作り、ユーザーを最初の入室者として登録します。

### 3. `updateWakeupTime`（起床時間の設定）

- **仕事内容:** 特定の部屋の「起床時間」を設定・変更します。
- **アプリとの連携:**
    1. 誰かがアプリの待機画面で起床時間を設定します。
    2. アプリが「この部屋の起床時間を朝7時にしてください」と、この`updateWakeupTime`という命令を呼び出します。
    3. サーバーは、データベースの`rooms`部屋リストにある、該当の部屋の`wakeupTime`を更新します。

### 4. `sendSignalingMessage`（伝言・通信の仲介）

- **仕事内容:** これが一番の働き者で、「伝言係」や「郵便屋さん」の役割をします。ユーザー間の様々なメッセージを仲介します。
- **アプリとの連携:**
    - **お仕置きの開始時 (WebRTC):**
    ビデオ通話の「申込書(offer)」や「返信書(answer)」などを、この命令を使ってサーバーの伝言板に貼り出します。
    - **遠隔操作時 (EV3):**
    操作者Aのアプリが操作ボタンを押すと、その内容を「操作メモ」としてこの命令を使って送ります。サーバーはそのメモを伝言板に貼り出し、寝坊者Cのアプリがそれを受け取ってEV3に命令します。
    - **【重要】具体的な操作メモのルール:**
    アプリ側では、以下のような共通のルールで「操作メモ」を作成して送信します。
        - **十字キー（車の操作）の場合:**
            - `type`: `"control"`
            - `data`: `{ "command": "move", "direction": "forward" }`
                - `direction` の部分を `"back"`, `"left"`, `"right"`, `"stop"` のように変えて送信します。
        - **お仕置きボタン（ハンマー）の場合:**
            - `type`: `"control"`
            - `data`: `{ "command": "action", "action": "hammer_strike" }`
        
        このようにルールを決めておけば、寝坊者Cのアプリは、受け取ったメモの`command`や`direction`を見て、どのBluetooth信号をEV3に送ればよいかを判断できます。
        

### 5. `checkWakeupTimes`（見回り）

- **仕事内容:** これはアプリから直接呼ばれるのではなく、**1分ごとに自動で実行される「見回り役」**です。
- **アプリとの連携:**
    1. Googleのシステムが、1分ごとにこの`checkWakeupTimes`という命令を叩き起こします。
    2. サーバーは、データベースにある全ての部屋を見回り、「設定された起床時間を過ぎている部屋はないか？」をチェックします。
    3. もし該当する部屋があれば、「この部屋は起床時間ですよ！」とログに記録します。（この記録を元に、アプリ側でアラームを鳴らすなどの処理が実行されます）

このように、アプリは「見た目」と「操作」を担当し、このサーバープログラムは裏側で「記録」「管理」「仲介」「時間チェック」といった、目に見えないけれど重要な仕事をすべて担当しているのです。